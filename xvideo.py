...
def cropimg(detections, image):
    ...
    for detection in detections:
        ...
        print('ricacdo: ',fine_tune(ricacdo))
...
def YOLO():
    ...
    while True:
        ...
        print(darknet.network_height(netMain))
        ...
        #grayscaled = cv2.cvtColor(bienso, cv2.COLOR_BGR2GRAY)
        #threshold = cv2.adaptiveThreshold(grayscaled, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 115, 1)
        #text = pytesseract.image_to_string(threshold,lang=None)
        #print('Bien so: ',text)
