...
def cvDrawBoxes(detections, img):
    for detection in detections:
        ...
        if image1.size !=0:
            ...
            thre_mor, cont, hierarchy= cv2.findContours(thre_mor, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cnts = imutils.grab_contours(cont)
            cnts = sorted(cnts, key=cv2.contourArea, reverse=True)
            cnts = sort_contours(cnts)
            #sorted_cont = sorted(cont, key=lambda ctr: cv2.boundingRect(ctr)[0]* image1.shape[0]+ cv2.boundingRect(ctr)[1] * image1.shape[1],reverse=False )
            #sorted_cont = sorted(cont, key=lambda ctr: cv2.contourArea(ctr),reverse=False )
            
            #sorted_cont = sorted(cont, key=lambda ctr:get_contour_precedence(ctr, image1.shape[1]))
            plate_info = ""

            for i, ctr in enumerate(sorted_cont):
                x, y, w, h = cv2.boundingRect(ctr)
                ratio = h/w
                if 1.5<=ratio<=3.5: # Chon cac contour dam bao ve ratio w/h
                    if 0.25<=h/roi.shape[0]<=0.8: 
